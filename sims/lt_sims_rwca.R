...
# Setting 2: binary treatment with trial arm (no LT) and rwCA (subject to LT)
...
GenerateFullDataset <- function(n,
  ...
  #   n: sample size
  #   p_trt: P(trt = 1)
  #   entry_rate: rate of entry event, calculated using CalculateEntryRate in order to satisfy given p_lt in rwCA
  #   p_preindex: probability of entry before index date (i.e. entry = 0) in rwCA
  ...
  n_rwCA <- round(n * (1 - p_trt))
  n_trial <- n - n_rwCA
    
  data_rwCA <- data.frame(entry = rexp(n_rwCA, rate = entry_rate)) %>%
    mutate(preindex_entry = rbinom(n_rwCA, 1, p_preindex),
           entry = entry * (1 - preindex_entry),
           trt = 0)
  
  data_trial <- data.frame(entry = rep(0, n_trial)) %>%
    mutate(preindex_entry = 1,
           trt = 1)
  
  data <- bind_rows(data_rwCA, data_trial) %>%
    mutate(lambda = baseline_hazard * exp(beta * trt + dependence * entry),
...
CalculateEntryRate <- function(n,
  ...
  #   n: sample size
  #   p_trt: P(trt = 1)
  #   p_lt: desired LT proportion in rwCA, can range from (0, 1 - p_preindex)
  #   p_preindex: probability of entry before index date (i.e. entry = 0) in rwCA
  #   dependence: parameter controlling dependence of survival time on entry time, on the log hazard ratio scale 
  #   baseline_hazard: baseline hazard of death, constant
  #   nreps: number of simulated datasets to compute empirical LT
  # Output: rate of entry event that will result in p_lt in rwCA
  
  lt_fn <- function(er) {
    
    lt_obs <- replicate(nreps, 
                        mean(GenerateFullDataset(n, p_trt, er, p_preindex, dependence, baseline_hazard) %>%
                               filter(trt == 0) %>%
                               pull(is_lt)))
...
FitModels <- function(data) {
  ...
  baseline_dfs <- map_dfr(c(0.25, .5, .75),
                          function(q) {
                            quant_obs <- quantile(naive[naive$trt == 0,]$entry, q)
                            
                            naive %>% 
                              filter(entry <= quant_obs) %>% 
...
RunSimulationSetting <- function(n,
  ...
  # Set rate of entry in order to satisfy given p_lt in rwCA
...
if (T) {
  ...
  save(results, file = "SimResults_rwca.RData")
}
